...
export class MetamaskHelper {
  public static async initDappeteer(seedPhrase: string = '', password: string = '') {
    let seed: string;
    let pass: string;
  ...
  private static async getMetamask(browser: DappeteerBrowser, seed: string, password: string) {
  ...
  private static async getBrowser(): Promise<DappeteerBrowser> {
    ...
    let options: DappeteerLaunchOptions = {
      ...
      puppeteerOptions: {
        ...
        slowMo: Boolean(process.env.CI) == true ? 2 : 1000,
        args: [
          '--no-sandbox',
